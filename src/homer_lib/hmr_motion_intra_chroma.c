...
void synchronize_motion_buffers_chroma(henc_thread_t* et, cu_partition_info_t* curr_cu_info, wnd_t * quant_src, wnd_t * quant_dst, wnd_t *decoded_src, wnd_t * decoded_dst, int gcnt)
{
	...
	for(comp=U_COMP;comp<=V_COMP;comp++)
	{
		...
		wnd_copy_cu_2D(et->funcs->sse_copy_16_16, curr_cu_info, decoded_src, decoded_dst, comp);
		wnd_copy_cu_1D(et->funcs->sse_copy_16_16, curr_cu_info, quant_src, quant_dst, comp);
	...
	for(cu_mode_idx=0;cu_mode_idx<NUM_CHROMA_MODE;cu_mode_idx++)
	{
		...
		while(!(curr_depth==(depth-(part_size_type==SIZE_NxN)) && depth_state[curr_depth]==(part_position&0x3)+1))
		{
			...
			for(ch_component = U_COMP;ch_component<=V_COMP;ch_component++)
			{
				...
				partition_cost[depth_state[curr_depth]] += (int)(weight*et->funcs->ssd16b(orig_buff, orig_buff_stride, decoded_buff, decoded_buff_stride, curr_part_size));//R-D
			}//for(ch_component = U_COMP;ch_component<=V_COMP;ch_component++)
