...
/*void hmr_bitstream_write2file(bitstream_t* bs)
{
	...
	{
		fprintf(stderr,"Fatal: cannot open Annex B bytestream file 'prueba.bin', exit (-1)\n");
	}
...
void hmr_bc_bitstream_write_bits_uvlc(bitstream_t* bs, unsigned int val)
{
	...
	hmr_bc_bitstream_write_bits(bs, 0,length>>1);
	hmr_bc_bitstream_write_bits(bs, val,(length+1)>>1);
}
...
void hmr_bc_bitstream_write_bits_svlc(bitstream_t* bs, int val)
{
	...
	hmr_bc_bitstream_write_bits_uvlc(bs, uval);
}
...
{
  if (bs->streambitcnt != 0)
    hmr_bc_bitstream_write_bits(bs, 0xff,(8-bs->streambitcnt));
}
...
{
  if (bs->streambitcnt != 0)
    hmr_bc_bitstream_write_bits(bs, 0,(8-bs->streambitcnt));
}

void hmr_bc_bitstream_rbsp_trailing_bits(bitstream_t* bs)
{
	hmr_bc_bitstream_write_bits(bs, 1,1);
...
{
	hmr_bc_bitstream_write_bits(bs,0,1);                    // forbidden_zero_bit
	hmr_bc_bitstream_write_bits(bs,nalu_type, 6);  // nal_unit_type
	hmr_bc_bitstream_write_bits(bs,rsvd_zero6bits, 6);                   // nuh_reserved_zero_6bits
	hmr_bc_bitstream_write_bits(bs,temporal_id+1, 3); // nuh_temporal_id_plus1
}
